...
public class TestController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String gugu = request.getParameter("num1"); //구구단
		String odds = request.getParameter("num2"); //홀수짝수
		
		if (gugu == null) {
			String result = model2.oper2(odds);
			request.setAttribute("data", result);	//홀수 짝수 판별 (모델부분)
			request.getRequestDispatcher("/WEB-INF/views/test2show.jsp").forward(request, response);
		} else if (odds == null) {
			ArrayList<String> list = model.oper1(gugu); //구구단 출력 (모델부분)
