...
public class OrderMgr {
	...
	// 여러 상품을 주문하니까 arraylist로
	public ArrayList<OrderBean> getOrder(String id){
		ArrayList<OrderBean> list = new ArrayList<OrderBean>();
		
		try {
			conn = ds.getConnection();
			String sql = "select * from shop_order where id=? order by no desc" ; // 최근 상품부터 보려고 함
	...
	public ArrayList<OrderBean> getOrderAll(){
		...
		try {
			conn = ds.getConnection();
			String sql = "select * from shop_order order by no desc" ; // 최근 상품부터 보려고 함
	...
	public OrderBean getOrderDetail(String no){
		OrderBean bean = null; // 상품 하나만 읽어야되 // 슬리퍼 하나만 고치게 
	...
	public boolean updateOrder(String no, String state) {
		...
		try {
			...
			if(pstmt.executeUpdate() > 0) b = true; // 업데이트 가 하나라도 있으면 
	...
	public boolean deleteOrder(String no) {
		...
		try {
			...
			if(pstmt.executeUpdate() > 0) b = true; // executeUpdate
